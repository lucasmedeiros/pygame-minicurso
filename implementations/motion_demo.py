...
class MotionSprite(pygame.sprite.Sprite):
    def __init__(self):
...
while running:
    ...
    draw_vectors(motion)
    txt = "Pos: ({:.2f}, {:.2f})".format(motion.pos.x, motion.pos.y)
    draw_text(txt, 25, WHITE, 5, 5)
    txt = "Vel: ({:.2f}, {:.2f})".format(motion.vel.x, motion.vel.y)
    draw_text(txt, 25, GREEN, 5, 55)
    txt = "Acc: ({:.2f}, {:.2f})".format(motion.acc.x, motion.acc.y)
